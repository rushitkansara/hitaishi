...
def load_and_prepare_data():
    ...
    X_train_full = np.load(config.DATA_PROCEDURAL_X_TRAIN_PATH)['X_train']
    y_train_full = np.load(config.DATA_PROCEDURAL_Y_TRAIN_PATH)
    
    # Load the procedural test data (for saving later, not for training)
    X_test = np.load(config.DATA_PROCEDURAL_X_TEST_PATH)['X_test']
    y_test = np.load(config.DATA_PROCEDURAL_Y_TEST_PATH)
    ...
    with open(config.DATA_PROCEDURAL_SCALER_PATH, 'rb') as scaler_file:
...
def run_training(model, X_train, y_train, X_val, y_val):
    ...
    # Save final model
    model.save(config.MODEL_V1_2_H5_PATH)
...
def save_model_metadata(model, X_train, y_train, history):
    ...
    metadata = {
        "model_name": "hitaishi_v1_2",
        "version": "1.2", # Unified v1.2 version
